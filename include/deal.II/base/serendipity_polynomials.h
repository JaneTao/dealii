...
#ifndef __deal2__serendipity_polynomials_h
#define __deal2__serendipity_polynomials_h
...
protected:
  ...
  /**
   * Number of serendipity product polynomials. See n().
   */
  unsigned int n_seren_pols;
  ...
  polynomials (pols.begin(), pols.end()),
{
  ...
  {
    n_seren_pols += (unsigned int)std::pow(2,dimension-i)* 
    Utilities::n_choose_k(dimension, i)*
    Utilities::n_choose_k(degree-i, i);
  }

  index_map.resize(n_seren_pols);
  index_map_inverse.resize(n_seren_pols);
...
inline
unsigned int
SerendipityPolynomials<dim,POLY>::n() const
{
  if (dim == 0)
...
SerendipityPolynomials<dim,POLY>::get_numbering() const
{
  return index_map;
}
...
inline
const std::vector<unsigned int> &
SerendipityPolynomials<dim,POLY>::get_numbering_inverse() const
{
