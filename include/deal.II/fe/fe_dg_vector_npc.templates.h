...
#include <deal.II/fe/fe_dg_vector_npc.h>
...
FE_DGVector_NPC<POLY,dim,spacedim>::FE_DGVector_NPC (
...
FE_DGVector_NPC<POLY,dim,spacedim>::clone() const
{
  return new FE_DGVector_NPC<POLY, dim, spacedim>(*this);
}


template <class POLY, int dim, int spacedim>
std::string
FE_DGVector_NPC<POLY,dim,spacedim>::get_name() const
{
  std::ostringstream namebuf;
  namebuf << "FE_DGVector_NPC_" << this->poly_space.name()
          << "<" << dim << ">(" << this->degree-1 << ")";
  return namebuf.str();
}
...
FE_DGVector_NPC<POLY,dim,spacedim>::get_dpo_vector (const unsigned int deg)
{
...
template <class POLY, int dim, int spacedim>
bool
FE_DGVector_NPC<POLY,dim,spacedim>::has_support_on_face (const unsigned int,
...
template <class POLY, int dim, int spacedim>
void
FE_DGVector_NPC<POLY,dim,spacedim>::interpolate(
  std::vector<double> &,
...
template <class POLY, int dim, int spacedim>
std::size_t
FE_DGVector_NPC<POLY,dim,spacedim>::memory_consumption() const
{
  Assert(false, ExcNotImplemented());
  return 0;
}
