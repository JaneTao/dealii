...
#include <deal.II/fe/fe_s_base.h>
...
struct FE_S_Base<POLY,xdim,xspacedim>::Implementation
{
  /**
                               ...
                               FE_S_Base<POLY,2,spacedim> &fe)
  {
    ...
    const std::vector<unsigned int> face_index_map =
      FE_S_Helper::face_lexicographic_to_hierarchic_numbering<dim>(fe.degree);
  ...
  static
  void initialize_constraints (const std::vector<Point<1> > &/*points*/,
                               FE_S_Base<POLY,3,spacedim> &fe)
  {
    const unsigned int dim = 3;

    // For a detailed documentation of the interpolation see the
    // FE_S_Base<2>::initialize_constraints function.
...
FE_S_Base<POLY,dim,spacedim>::FE_S_Base (const POLY &poly_space,
...
template <class POLY, int dim, int spacedim>
void
FE_S_Base<POLY,dim,spacedim>::initialize (const std::vector<Point<1> > &points)
{
  Assert (points[0][0] == 0,
                          ...
                          FullMatrix<double>       &interpolation_matrix) const
{
  ...
  if (const FE_S_Base<POLY,dim,spacedim> *source_fe
      = dynamic_cast<const FE_S_Base<POLY,dim,spacedim>*>(&x_source_fe))
    {
...
template <class POLY, int dim, int spacedim>
bool
FE_S_Base<POLY,dim,spacedim>::hp_constraints_are_implemented () const
{
  return false;
}
...
template <class POLY, int dim, int spacedim>
void FE_S_Base<POLY,dim,spacedim>::initialize_unit_support_points (const std::vector<Point<1> > &points)
{
  // const std::vector<unsigned int> &index_map_inverse=
...
template <class POLY, int dim, int spacedim>
void FE_S_Base<POLY,dim,spacedim>::initialize_unit_face_support_points (const std::vector<Point<1> > &points)
{
  ...
  //   FE_S_Helper::face_lexicographic_to_hierarchic_numbering<dim>(this->degree);
  // Quadrature<1> support_1d(points);
...
template <class POLY, int dim, int spacedim>
void
FE_S_Base<POLY,dim,spacedim>::initialize_quad_dof_index_permutation ()
{
...
template <class POLY, int dim, int spacedim>
std::vector<unsigned int>
FE_S_Base<POLY,dim,spacedim>::get_dpo_vector(const unsigned int deg)
{
...
template <class POLY, int dim, int spacedim>
void
FE_S_Base<POLY,dim,spacedim>::initialize_constraints (const std::vector<Point<1> > &points)
{
  ...
  if (this->prolongation[refinement_case-1][child].n() == 0)
    {
      ...
      FE_S_Helper::zero_indices<dim> (j_indices);
      for (unsigned int j=0; j<q_dofs_per_cell; j+=dofs1d)
        {
          unsigned int i_indices[dim];
          FE_S_Helper::zero_indices<dim> (i_indices);
          for (unsigned int i=0; i<q_dofs_per_cell; i+=dofs1d)
            {
              ...
              FE_S_Helper::increment_indices<dim> (i_indices, dofs1d);
            }
          Assert (i_indices[dim-1] == 1, ExcInternalError());
          FE_S_Helper::increment_indices<dim> (j_indices, dofs1d);
        }
...
template <class POLY, int dim, int spacedim>
bool
FE_S_Base<POLY,dim,spacedim>::has_support_on_face (const unsigned int shape_index,
...
FE_S_Base<POLY,dim,spacedim>::get_constant_modes () const
{
